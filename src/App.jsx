...
const MODEL_PATH = "./brain_tumor_model.onnx"; 

const IMAGE_SIZE = 224; 

const CLASSES = ["Glioma", "Meningioma", "No Tumor", "Pituitary"];

// --- DIAGNOSTIC: Try to detect model input format ---
let modelInputShape = null;

// --- 1. IMAGE PREPROCESSING (FLEXIBLE FORMAT) ---
async function preprocessImage(imageFile, useChannelsFirst = false) {
  return new Promise((resolve) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const float32Data = new Float32Array(1 * IMAGE_SIZE * IMAGE_SIZE * 3);
        
        if (useChannelsFirst) {
          // PyTorch format: [1, 3, 224, 224]
          console.log("Using channels-first format (PyTorch)");
          ...
          console.log("Using channels-last format (TensorFlow)");
          for (let i = 0; i < IMAGE_SIZE * IMAGE_SIZE; ++i) {
            const r = imgData[i * 4] / 255.0;
            const g = imgData[i * 4 + 1] / 255.0;
            const b = imgData[i * 4 + 2] / 255.0;
          ...
          const inputTensor = new ort.Tensor('float32', float32Data, [1, IMAGE_SIZE, IMAGE_SIZE, 3]);
          resolve(inputTensor);
        }
      };
...
// --- 2. INFERENCE ENGINE (AUTO-DETECT FORMAT) ---
const runInference = async (imageFile) => {
  try {
    ...
    // Try to detect expected shape
    if (inputMetadata && inputMetadata[inputName]) {
      const expectedShape = inputMetadata[inputName].dims;
      console.log("Expected input shape:", expectedShape);
      modelInputShape = expectedShape;
    }
    
    // Determine format based on shape
    let useChannelsFirst = false;
    if (modelInputShape) {
      // If shape is [1, 3, 224, 224] -> channels first (PyTorch)
      // If shape is [1, 224, 224, 3] -> channels last (TensorFlow)
      if (modelInputShape[1] === 3) {
        useChannelsFirst = true;
        console.log("🔍 Detected PyTorch format [1, 3, 224, 224]");
      } else if (modelInputShape[3] === 3) {
        useChannelsFirst = false;
        console.log("🔍 Detected TensorFlow format [1, 224, 224, 3]");
      }
    }

    console.log("Preprocessing image...");
    const inputTensor = await preprocessImage(imageFile, useChannelsFirst);
    ...
    console.log("Raw output:", Array.from(outputData));
    ...
    console.log("✅ Prediction:", CLASSES[maxIndex], "| Confidence:", (maxScore * 100).toFixed(2) + "%");
    ...
    if (e.message.includes("invalid dimensions")) {
      errorMsg += "❌ Input shape mismatch!\n\n";
      errorMsg += "The error suggests the model expects a different input format.\n\n";
      errorMsg += "Details from error:\n" + e.message + "\n\n";
      errorMsg += "This usually means:\n";
      errorMsg += "1. The model was exported with a specific input shape\n";
      errorMsg += "2. We need to match that exact shape\n\n";
      errorMsg += "Check the console for detected model shape.";
    } else if (e.message.includes("Failed to fetch") || e.message.includes("404")) {
      errorMsg += "❌ Model file not found!\n\n";
      errorMsg += "Ensure 'brain_tumor_model.onnx' is in the 'public' folder.";
    } else {
      errorMsg += e.message;
    }
  ...
  <footer className="bg-white border-t border-slate-200 mt-auto">
    <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-center text-sm text-slate-500">
      <div className="flex items-center gap-2">
        ...
        <span> Deep Learning Course Project</span>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 animate-fade-in">
      <div className="text-center mb-10 space-y-2">
        <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">AI-Powered Brain Tumor Classification</h1>
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-10 text-white">
            ...
            <p className="text-slate-300 leading-relaxed text-lg max-w-2xl">
                The Brain Tumor Classification System (BTCS) is a Deep Learning Course Project dedicated to leveraging Artificial Intelligence for healthcare advancement.
            </p>
        </div>
